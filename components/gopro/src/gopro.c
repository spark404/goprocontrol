    ...
    for(;;) {
        ...
        switch (connection->connection_state) {
            ...
            case GOPRO_HTTP_DISCONNECTED:
                if (gopro_http_get_info(&gopro_info) == ESP_OK) {
                    ESP_LOGD(TAG, "HTTP info retrieved, state set to GOPRO_WIFI_CONNECTED");
                    connection->camera_name = gopro_info.model_name;
                    connection->camera_mac = gopro_info.mac_address;
                ...
                if (wol_retries < 5 && connection->camera_mac != NULL) {
                    ...
                    ESP_LOGW(TAG, "No response from camera despite wakeup requests, resetting connection");
//                    if (gopro_info.model_name != NULL) {
//                        free(gopro_info.model_name);
//                        gopro_info.model_name = NULL;
//                        free(gopro_info.mac_address);
//                        gopro_info.mac_address = NULL;
//                    }
                    gopro_http_cleanup();
...
esp_err_t gopro_init(gopro_connection_t *gopro_connection) {
    ...
    ESP_LOGI(TAG, "Initializing WiFi connection to GoPro");
     ...
     */
    if (strlen(GOPRO_ESP_MAC_ADDRESS) > 0) {
        gopro_connection->camera_mac = GOPRO_ESP_MAC_ADDRESS;
    }
